const Testimonils = () => {
  const testimonials = [
    ...
    },
    {
      ...
      title: 'Banker',
      quote:
        'whatever service i need, I can find them all on Homygig.  The variety of services offered is fantastic, and it saves me the trouble of searching for different companies for each job'
    }
  ];
      ...
      <div className="relative  z-10 max-w-screen-xl mobile:max-sm:px-5 mx-auto px-24 md:px-16">
        <div className="max-w-xl sm:text-center  md:mx-auto mobile:max-sm:text-center">
          ...
          <p className="mt-3 text-gray-600">
            Ready to experience the homygig difference for yourself? Join the many satisfied
            customers who have enjoyed a seamless experience!
          </p>
        </div>
        <div className="mt-12 ">
          <ul className="grid items-center gap-10  sm:grid-cols-2 lg:grid-cols-3">
            {testimonials.map((item, idx) => (
              <li key={idx} className="bg-white rounded-xl border shadow-md">
                ...
                <figure>
                  <blockquote>
                    <p className="text-gray-800 text-sm  px-4 py-1">{item.quote}</p>
                  </blockquote>
                  <div className="flex items-center gap-x-4 p-4 mt-6 ">
                    ...
                    <div>
                      ...
                      <span className="block text-primarytheme text-sm mt-0.5">{item.title}</span>
                    </div>
                  </div>
                </figure>
              </li>
