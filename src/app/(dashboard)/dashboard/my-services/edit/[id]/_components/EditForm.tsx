'use client';
import ImageUpLoader from '@/core/components/organisms/ImageUpLoader';
...
const EditServiceForm = () => {
  ...
  const handleUpdate = async () => {
    ...
    if (service && service.id) {
      if (!name || !price || !category_id || !description || !product_image) {
      ...
      const updatedData = await Queries.updateService(service.id, serviceData);
      if (updatedData && updatedData.id) {
        ...
        toast.error(`failed to update service!} ${updatedData.message.toString()})}`, {
          position: 'top-right',
          hideProgressBar: true,
          closeOnClick: true
        });
        ...
        <div>
          ...
          <div className=" h-full flex justify-between mobile:max-lg:flex-col-reverse gap-5 py-4 px-4 bigScreen:px-40 bigScreen:py-20">
            ...
            <div className=" w-[50%] mobile:max-md:w-full border rounded-b-sm mobile:max-md:px-4 mobile:max-md:justify-center  flex justify-end ">
              {service && (
                <ImageUpLoader
                  onUpdateMainImage={(image) => setProduct_image(image)}
                  onImagesUpload={(images) => setImages(images)}
